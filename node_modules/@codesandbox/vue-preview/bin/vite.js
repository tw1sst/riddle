...
function __proxyExport(rawOptions = {}) {
  ...
  rawOptions.template.compilerOptions.nodeTransforms.push((node, ctx) => {
    if (node.type === 1) {
        ...
        addEvent(node, 'pointerenter', \`$event ? $volar.highlight($event.target, $.type.__file, [\${startLine},\${endLine},\${startColumn},\${endColumn}]) : undefined\`);
        addEvent(node, 'pointerleave', '$event ? $volar.unHighlight($event.target) : undefined');
        addEvent(node, 'vnode-mounted', \`$event ? $volar.vnodeMounted($event.el, $.type.__file, [\${startLine},\${endLine},\${startColumn},\${endColumn}]) : undefined\`);
...
`;
  ...
  console.warn(
    "Volar: @vitejs/plugin-vue not found, skip vite extra code. (vue@2 only)"
  );
}
...
function createViteConfig() {
  let proxyConfigContent = fs.readFileSync(
    path.resolve(__dirname, "vite", "config.ts"),
    { encoding: "utf8" }
  );
  proxyConfigContent = proxyConfigContent.replace(
    "{CONFIG_PATH}",
    JSON.stringify(path.resolve(workspace, "vite.config"))
  );
  ...
  const proxyConfigPath = path.resolve(
    workspace,
    "node_modules",
    ".volar",
    "vite.config.ts"
