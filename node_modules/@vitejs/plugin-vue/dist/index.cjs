...
function __proxyExport(rawOptions = {}) {
  ...
  rawOptions.template.compilerOptions.nodeTransforms.push((node, ctx) => {
    if (node.type === 1) {
      ...
      addEvent(node, 'pointerenter', `$event && $volar ? $volar.highlight($event.target, $.type.__file, [${startLine},${endLine},${startColumn},${endColumn}]) : undefined`);
      addEvent(node, 'pointerleave', '$event && $volar ? $volar.unHighlight($event.target) : undefined');
      addEvent(node, 'vnode-mounted', `$event && $volar ? $volar.vnodeMounted($event.el, $.type.__file, [${startLine},${endLine},${startColumn},${endColumn}]) : undefined`);
      addEvent(node, 'vnode-unmounted', '$event && $volar ? $volar.vnodeUnmounted($event.el) : undefined');
    }
  });
